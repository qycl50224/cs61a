...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        atta = other_card.attack
        defen = other_card.defense
        for i in range(len(player.deck.cards)):
            player.deck.cards[i].attack += atta
            player.deck.cards[i].defense += defen
        for card in opponent.deck.cards[:]:
            if card.attack == atta or card.defense == defen:
