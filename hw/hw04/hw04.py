...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    l_left = length(left(m))
    l_right = length(right(m))   
    # print(l_left)
    if is_weight(end(left(m))) and is_weight(end(right(m))):
        # print(l_left)
        w_left = size(end(left(m)))
        w_right = size(end(right(m)))
        result = (l_left * w_left == l_right * w_right)
        # print('result',result)
        return result 
    else:
        if is_mobile(end(left(m))):
            w_left = total_weight(end(left(m)))
            if is_mobile(end(right(m))):
                w_right = total_weight(end(right(m)))
                # print('left',balanced(end(left(m))))
                # print('right', balanced(end(right(m))))
                result = (l_left * w_left == l_right * w_right)
            ...
            result = (l_left * w_left == l_right * w_right)
            # print('result',result)
            return result and balanced(end(left(m)))
        w_left = size(end(left(m)))
        w_right = total_weight(end(right(m)))    
        # print('left',balanced(end(left(m))))
        # print('right', balanced(end(right(m))))
        result = (l_left * w_left == l_right * w_right)
        # print('result',result)
        return result and balanced(end(right(m)))
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        result = [total_weight(m),totals_tree(end(left(m))),totals_tree(end(right(m)))]
        # print(result)
        return result
...
def replace_leaf(t, old, new):
    ...
    # print(t[0])
    # print(t[1:])
    # a = [(t[0])] + list(t[1:])    # print(a)
    # print(a)
    def helper(t):
        if is_leaf(t) and label(t) == old:
            return tree(new)    
        else:
            bs = [helper(branch) for branch in branches(t)]
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    def with_draw(amount, password):
        # print(old_password)
        # print(new_password)
        if password == new_password: # 适用于新密码
            return withdraw(amount, old_password) 
        return withdraw(amount, password) # 适用于最开始的密码
